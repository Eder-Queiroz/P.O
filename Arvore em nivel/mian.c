...
void PrintarNivelDaInfo(Noavl* tree, int level);
void OrdemDoNivel(Noavl* tree);
int altura(Noavl* tree);
...
void PrintarNivelDaInfo(Noavl* tree, int level) {
    ...
    if(level == 1) {
        ...
        PrintarNivelDaInfo(tree->esq, level - 1);
        PrintarNivelDaInfo(tree->dir, level - 1);
    }
...
void OrdemDoNivel(Noavl* tree) {
    ...
    int altura_info = altura(tree);
    int i;

    for(i = 1; i <= altura_info; i++) {

        PrintarNivelDaInfo(tree, i);
        printf("\n");   
    }
}
...
int altura(Noavl* tree) {
    ...
    if(tree == NULL) {
        ...
        int altura_esquerda = altura(tree->esq);
        int altura_direita = altura(tree->dir);

        int altura_final = (altura_esquerda > altura_direita ? altura_esquerda : altura_direita) + 1;
